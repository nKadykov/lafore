...
std::istream& operator>>(std::istream& s, Fraction& f) {
    ...
    while(true) { // бесконечный цикл
        ...
        s >> f.dividend;
        if(!s.good()) { // проверка, является ли числитель числом
            s.clear();
            std::cout << "Incorrect dividend\n";
            s.ignore(80, '\n');
            continue;
        }
        s >> ch;
        if(ch != '/') { // проверка, является ли символ '/'
            std::cout << "Incorrect fraction\n";
            s.ignore(80, '\n');
            continue;
        }
        s >> f.divisor;
        if(!s.good()) { // проверка, является ли знаменатель числом
            s.clear();
            std::cout << "Incorrect divisor\n";
            s.ignore(80, '\n');
            continue;
        }
        if(f.divisor == 0) { // проверка, является ли знаменатель не 0
            std::cout << "Divisor can't be 0\n";
            s.ignore(80, '\n');
            continue;
        }
        else { // выйти из цикла, если все условия соблюдены
            break;
        }
    }
    return s;
